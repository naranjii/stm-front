...
export default function TaskDashboard() {
  ...
  const alternarConcluida = async (id, concluida) => {
    try {
      ...
      await fetch(`${import.meta.env.VITE_API_URL}/api/tasks/${id}`, {
        ...
        body: JSON.stringify({ status: !concluida ? 'concluida' : 'pendente' }),
      });
      carregarTasks();
    } catch {
  ...
  return (
      <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 border-2 border-green-300 shadow-2xl bgbox text-green-700 font-medium rounded-xl p-4 max-w-x2 mx-auto space-y-4 mt-6">
        <h2 className="text-[25px] font-bold text-center underline tracking-[5px] underline-offset-12">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;🍃Task List🍃&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</h2>                                    
            ...
            <li key={task._id} className={task.concluida ? 'taskdone h-11 m-2 flex items-center justify-between bg-transparent p-2 border rounded-xl shadow' : 'h-11 m-2 flex items-center justify-between bg-transparent p-2 border rounded-xl shadow'}>
              <div>
                <input
                  ...
                  onChange={() => alternarConcluida(task._id, task.concluida)}
